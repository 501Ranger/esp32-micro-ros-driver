...
<body>
  <main>
    ...
    <form id="form">
      ...
      <div class="button-group">
        ...
        <button class="secondary" type="button" id="calibrate" style="background: rgba(99, 102, 241, 0.15); border-color: rgba(99, 102, 241, 0.4); color: #a5b4fc;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  <script>
    ...
    document.getElementById('calibrate').addEventListener('click', async () => {
      if (confirm('确定要开始校准 IMU 陀螺仪零偏吗？请确保机器人当前处于完全静止状态！')) {
        showMsg('正在执行 IMU 校准，请保持机器人完全静止...', 'success');
        try {
          ...
          if (res.status === 200) {
            ...
            showMsg('校准失败: ' + text, 'error');
          }
        } catch (err) {
          showMsg('发送校准请求失败', 'error');
        }
      }
    });
    renderHistory();
